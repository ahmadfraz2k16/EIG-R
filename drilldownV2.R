...
dataParentLayerHydro <- javascriptArray$DataForParentOne$DataParentLayerHydro
dataParentLayerNuclear <- javascriptArray$DataForParentOne$DataParentLayerNuclear
dataParentLayerThermal <- javascriptArray$DataForParentOne$DataParentLayerThermal
dataParentLayerRenewable <- javascriptArray$DataForParentOne$DataParentLayerRenewable
dataParentLayerIPPS <- javascriptArray$DataForParentOne$DataParentLayerIPPS
dataParentLayerGENCOS <- javascriptArray$DataForParentOne$DataParentLayerGENCOS
# Initialize a list to store the desired output
desiredOutputHydro <- list()
desiredOutputRenewable <- list()
desiredOutputNuclear <- list()
desiredOutputThermal <- list()
desiredOutputIPPS <- list()
desiredOutputGENCOS <- list()
# Loop through the data and push each entry to the desired output list
for (date in names(dataParentLayerHydro)) {
  desiredOutputHydro[[date]] <- dataParentLayerHydro[[date]]
}
# Loop through the data and push each entry to the desired output list
for (date in names(dataParentLayerRenewable)) {
  desiredOutputRenewable[[date]] <- dataParentLayerRenewable[[date]]
}
# Loop through the data and push each entry to the desired output list
for (date in names(dataParentLayerNuclear)) {
  desiredOutputNuclear[[date]] <- dataParentLayerNuclear[[date]]
}
# Loop through the data and push each entry to the desired output list
for (date in names(dataParentLayerThermal)) {
  desiredOutputThermal[[date]] <- dataParentLayerThermal[[date]]
}
# Loop through the data and push each entry to the desired output list
for (date in names(dataParentLayerGENCOS)) {
  desiredOutputGENCOS[[date]] <- dataParentLayerGENCOS[[date]]
}
# Loop through the data and push each entry to the desired output list
for (date in names(dataParentLayerIPPS)) {
  desiredOutputIPPS[[date]] <- dataParentLayerIPPS[[date]]
}

# Initialize a highchart
hc <- highchart() %>%
  hc_chart(type = "column") %>%
  hc_title(text = "Energy Data") %>%
...
#     )
#   ) %>%
#   hc_title(text = "drilldown for all major categories") %>%
#   hc_xAxis(type = "category") %>%
#   hc_tooltip(enabled = TRUE) %>%
#   hc_plotOptions(series = list(stacking = "normal")) %>%
#   hc_yAxis(max = 160) %>%
