...
print(space_comparison)


#remove rows containing FUT
df<-df[-grep("FUT", df$power_plant_names), ]
# filling NA in first column with immediate power plant name above
df <- df %>%
  fill(power_plant_names, .direction = "down")
# Convert specific columns to numeric type (assuming columns from 4th to last are numeric)
# Adjust the column indices accordingly if the numeric columns are in different positions.from 3rd columns onwards to the right
df[, 3:length(df)] <- lapply(df[, 3:length(df)], as.numeric)
# seeing structure of data to its data types column wise
str(df)

```
<!-- converting above cell as a function to make it usable for multiple files -->
```{r}
# Function to read Excel file and clean data
clean_data <- function(file_path, filename) {
  # Read the Excel file
  df <- read_excel(file.path(file_path, filename), sheet = "MAIN", range = "A4:AY311")
...
# # Remove rows with specific names in the first column
#   names_to_remove <- c("TOTAL", "SMALL HYDEL", "TOTAL (SMALL HYDEL)", 
#                        "WAPDA HYDEL MVAR", "WAPDA HYDEL MW", "44627",
#                        "POWER PLANT", "GENCO-I", "THERMAL", "TOTAL JAMSHORO",
#                        "TOTAL LAKHRA", "TOTAL GENCO-I MVAR", "TOTAL GENCO-I MW",
#                        "GENCO-II", "TOTAL KOTRI", "GUDDU - 747", 
#                        "TOTAL GENCO-II MVAR", "TOTAL GENCO-II MW", 
#                        "GENCO-III", "TOTAL GENCO-III MVAR", "TOTAL GENCO-III MW",
#                        "TOTAL GENCOS MVAR", "TOTAL GENCOS MW", "TOTAL GENCOS MW",
#                        "TOTAL GENCOS MW", "PRIVATE POWER", "PRIVATE POWER",
#                        "IPPs HYDEL", "TOTAL IPPs HYDEL", "IPPs FOSSIL FUEL")
#   
#   df <- df %>%
#     filter(!power_plant_names %in% names_to_remove)
  ...
  df[, 3:length(df)] <- lapply(df[, 3:length(df)], as.numeric)
  # # Remove rows where unit_no column has values "NA", "UNIT", or "NO."
  # df <- df %>%
  #   filter(!unit_no %in% c("NA", "UNIT", "NO."))
...
# Usage of the function with your directory and filename
filename <- "07 MARCH 2K22.xlsm"
file_path <- "C:/Users/python/Documents/projR/MAR-2022"
main_worksheet <- clean_data(file_path, filename)
...
# See the structure of the cleaned data frame
# str(cleaned_df)
main_worksheet
...
filename <- "07 MARCH 2K22.xlsm"
file_path <- "C:/Users/python/Documents/projR/MAR-2022"
ENRG1_worksheet <- clean_data(file_path, filename)

```
<!-- observing dataframe -->
```{r eval=FALSE, include=FALSE}
write.csv(first_column, file = "power_stations_names.csv", row.names = FALSE)
...
clean_data <- function(file_path, filename) {
  ...
  values <- c("TOTAL IPPs  HYDEL
","TOTAL IPPs FOSSIL FUEL"
...
filename <- "07 MARCH 2K22.xlsm"
file_path <- "C:/Users/python/Documents/projR/MAR-2022"
DLR2_worksheet <- clean_data(file_path, filename)
str(DLR2_worksheet)
write.csv(DLR2_worksheet$power_stations, file = "power_stations_names.csv", row.names = FALSE)
```
```{r}
str(main_worksheet)
str(DLR2_worksheet)
str(ENRG1_worksheet)
```
<!-- CLEANING MW WORKSHEET USING RELEVANT.Rmd file code -->
```{r}
# Function to read Excel file and clean data
clean_data <- function(file_path, filename) {
  # Read the Excel file
...
filename <- "07 MARCH 2K22.xlsm"
file_path <- "C:/Users/python/Documents/projR/MAR-2022"
MW_worksheet <- clean_data(file_path, filename)
# str(DLR2_worksheet)
# write.csv(DLR2_worksheet$power_stations, file = "power_stations_names.csv", row.names = FALSE)
```
<!-- PREVIOUS CODE FOR CLEANING MW WORKSHEET -->
```{r}
...
filename <- "07 MARCH 2K22.xlsm"
file_path <- "C:/Users/python/Documents/projR/MAR-2022"
MW_worksheet <- process_mw_sheets(file_path, filename)
```
<!-- 10 categories based on MW worksheet categorization -->
```{r}
# # Function to extract a category based on start and end values, except wind category, wind category doesn't require 
# extract_category <- function(data frame, start_value, end_value, category_name) {
#   # Find the index of the starting point
#   start_index <- which(data frame$name == start_value)[1]
#   
#   # Find the index of the ending point
#   end_index <- which(data frame$name == end_value)[1]
#   
#   # Select the rows between start_index and end_index (inclusive)
#   category_df <- data frame[start_index:end_index, ]
#   
...
```
<!-- converting it into a single function -->
```{r}
# Function to extract a category based on start and end values
extract_category <- function(dataframe, start_value, end_value = NULL, category_name) {
  # Find the index of the starting point
...
process_mw_sheets <- function(fetched_path, fetched_file_name) {
  ...
  output_csv_path <- "C:/xampp/htdocs/latest_Dash/worksheets" 
  # Save the dataframe to a CSV file
  csv_file_name <- paste(output_csv_path, "MW.csv", sep = "/")
  write.csv(MW_worksheet_with_categories, csv_file_name, row.names = FALSE)
