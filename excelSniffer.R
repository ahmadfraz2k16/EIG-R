...
while (TRUE) {
  filess <- list.files(input_folder, pattern = "\\.(xlsx|xlsm)$", full.names = TRUE)
  ...
  if (length(filess) > 0) {
    ...
    path <- "C:/xampp/htdocs/latest_Dash/html/iconbar/include/csv"
    ...
    # reseting global_df
    global_df <- global_df[0, ]
    cat("No Excel files found. Waiting for files...\n")
    Sys.sleep(30)  
  }
}
